...
class IrisPipelineSklearnEndToEndTest(tf.test.TestCase):
  ...
  def setUp(self):
    ...
    self._blessing_path = os.path.join(self._experimental_root, 'data',
                                       'blessing')
  ...
  def testIrisPipelineSklearn(self, mock_runner):
    BeamDagRunner().run(
        iris_pipeline_sklearn._create_pipeline(
            ...
            blessing_path=self._blessing_path,
    ...
    expected_execution_count = 10  # 8 component + 1 resolver + 1 importer
