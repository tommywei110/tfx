...
class Driver(base_driver.BaseDriver):
  ...
  def resolve_exec_properties(
    ...
    input_path = exec_properties['input']
    absl.logging.debug('Processing input %s.' % input_path)
  ...
  def _prepare_output_artifacts(
    ...
    assert len(result) == 1, 'Multiple output artifacts are not supported.'
    ...
    example_artifact.set_string_custom_property(utils.FINGERPRINT_PROPERTY_NAME,
                                                exec_properties['fingerprint'])
    example_artifact.set_string_custom_property(utils.SPAN_PROPERTY_NAME,
                                                exec_properties['select_span'])
