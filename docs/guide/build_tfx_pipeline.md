...
A TFX pipeline is an instance of the
...
*   <var>pipeline-name</var>: The name of this pipeline. The pipeline name must
    be unique.
*   <var>pipeline-root</var>: The root path of this pipeline's outputs.
*   <var>components</var>: (Optional.) A list of component instances that make
    up this pipeline's workflow.
*   <var>enable-cache</var>: (Optional.) A boolean value that indicates if this
    pipeline uses caching to speed up pipeline execution.
*   <var>metadata-connection-config</var>: (Optional.) The connection
    configuration for ML Metadata.
*   <var>beam_pipeline_args</var>: (Optional.) A list of arguments for the
    Apache Beam pipeline orchestrator.
...
file and output `tf.Example` records. The `StatisticsGen` standard component
accepts `tf.Example` records as input and this component outputs dataset
statistics. So, an instance of `StatisticsGen` must be executed `ExampleGen`.

You can also define task-based dependencies using the
...
methods of your component. `add_upstream_node` lets you specify that the current
component must be executed after the specified component. `add_downstream_node`
lets you specify that the current component must be executed before the
specified component.

Continuing with the previous example, the following sample adds a task-based
dependency to ensure that `StatisticsGen` is executed after `ExampleGen`.
...
# Adds task-based dependency to ensure that StatisticsGen executes after SchemaGen
statistics_gen.add_upstream_node(example_gen)
...
Typically, it is not necessary to specify task-based dependencies since TFX
pipelines define the execution graph based on artifact dependencies between
component instances. In the preceding example, `StatisticsGen` would have
executed after `ExampleGen` anyway since `StatisticsGen` depends on an artifact
created by `ExampleGen`.
...
1.  Explore the files that were copied to your pipeline from the template. The
    ...
    *   A **models** directory with preprocessing scripts and model
        implementations using `tf.estimators` and Keras.
    *   A **pipeline** directory with pipeline implementation and configuration
        scripts.
    *   The template copies the following into the destination path: DAG runner
        scripts for Apache Beam and Kubeflow Pipelines, and notebooks to explore
        the artifacts in the [ML Metadata](mlmd) store.
...
1.  Design your pipeline. The scaffolding that a template provides helps you
    ...
    revise your code to make full use of TFX standard components. You may also
    need to create [custom components](understanding_custom_components) that
    implement features such as data augmentation.
...
1.  Once you have designed your pipeline, iteratively customize the pipeline
    ...
    your pipeline. For the **taxi** template, The `ExampleGen` component ingests
    ...
    1.  Run the component locally using the `beam_dag_runner.py` script. If the
...
1.  Design your pipeline. The TFX standard components provide proven
    ...
    *   Learn more about
        [standard TFX components](guide#tfx_pipeline_components).
...
1.  Create a script file to define your pipeline using the following example.
    ...
    1.  Run the component locally using Apache Beam by running the script file.
        If the script fails, debug the failure and retry running the script.
...
1.  Add the first node in your workflow to your pipeline. In this example, the
    ...
    `CsvExampleGen` creates `tf.Example` records using the data in the CSV at
    the specified data path. By setting the `CsvExampleGen` component's `input`
    parameter with
